...
from Tools_Face_Detection_System.predictor_tflite import PosPrediction
import Tools_Face_Detection_System.Face_Detection_Models as FD
...
class PRN():
    ...
    '''
    def __init__(self, is_dlib = False, prefix = './Tools_Face_Detection_System/'):
        ...
        #if is_dlib:
            #import dlib
            #detector_path = os.path.join(prefix, 'DataPrnet/net-data/mmod_human_face_detector.dat')
            #self.face_detector = dlib.cnn_face_detection_model_v1(
                    #detector_path)

        #---- load PRN 
        self.pos_predictor = PosPrediction(self.resolution_inp, self.resolution_op)
        #prn_path = "./Tools_Face_Detection_System/PRnet_TFLite_models/PRnet_pose_model_int8.tflite"
        prn_path = "./Tools_Face_Detection_System/PRnet_TFLite_models/PRnet_pose_model_f32.tflite"
        #prn_path = "./Tools_Face_Detection_System/PRnet_TFLite_models/PRnet_pose_model_f16.tflite"
        #prn_path = "./Tools_Face_Detection_System/PRnet_TFLite_models/PRnet_pose_model_int8.tflite"
        self.pos_predictor.restore(prn_path)

        # uv file
        self.uv_kpt_ind = np.loadtxt(prefix + '/DataPrnet/uv-data/uv_kpt_ind.txt').astype(np.int32) # 2 x 68 get kpt
        self.face_ind = np.loadtxt(prefix + '/DataPrnet/uv-data/face_ind.txt').astype(np.int32) # get valid vertices in the pos map
        self.triangles = np.loadtxt(prefix + '/DataPrnet/uv-data/triangles.txt').astype(np.int32) # ntri x 3
        
        self.uv_coords = self.generate_uv_coords()   
        
        
        model_direction = "./Tools_Face_Detection_System/MobilenetModels/ssd_int8_cpu.tflite"
            
        self.Mobilenetboxes_router = FD.Mobilenetboxes(model_direction)
    ...
    def dlib_detect(self, image,  model = "mobilenet"):
        #return self.face_detector(image, 1)
        if model == "mobilenet":
            model_direction = "./Tools_Face_Detection_System/MobilenetModels/ssd_int8_cpu.tflite"
            print(tcolors.RED, "4", image.dtype, tcolors.ENDC)
            face, points = self.Mobilenetboxes_router.call(image)
            y_min,y_max, x_min,x_max = points
            points = [x_min,x_max,y_min,y_max ]
        elif model == "lbpcascade":
            face, points = FD.face_detection_with_Haarcacade(image)
            y_min,y_max, x_min,x_max = points
            points = [x_min,x_max,y_min,y_max ]
    ...
    def process(self, input, image_info = None):
        ...
        # image = image/255.
        image = np.array(tf.cast(image, dtype="float32")/255.0)
